...
/* ---------- TEMPORARY: one-time remote bootstrap ----------
   Local machines here can't reach the Postgres database directly (port
   5432 is being blocked somewhere on the way out), but Vercel's own
   servers can. These two routes let the initial seed + legacy-data
   migration run FROM Vercel instead, triggered remotely over normal
   HTTPS. Protected by SESSION_SECRET so only the site owner can call
   them. DELETE THIS WHOLE BLOCK once the migration is done. */
const fs = require('fs');
function bootstrapAuth(req, res, next) {
  if (req.headers['x-bootstrap-secret'] !== process.env.SESSION_SECRET) return res.status(403).json({ error: 'forbidden' });
  next();
}

app.post('/api/_bootstrap/seed', bootstrapAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, adminCreated, categoriesSeeded: nCat, productsSeeded: nProd });
  } catch (err) {
    console.error(err);
...
app.post('/api/_bootstrap/migrate', bootstrapAuth, async (req, res) => {
  try {
    const { users, categories, products, content, reviews, bookings, orders } = req.body || {};
    ...
    for (const c of (categories || [])) {
      await sql`INSERT INTO categories (dept, name, photo, sale, sort) VALUES (${c.dept}, ${c.name}, ${c.photo}, ${c.sale}, ${c.sort})
    ...
    for (const p of (products || [])) {
      await sql`INSERT INTO products (slot_id, dept, name, price, was, badge, category, colours, blurb, photo, stock, active, barcode, vendor_barcode_photo)
        VALUES (${p.slot_id}, ${p.dept}, ${p.name}, ${p.price}, ${p.was}, ${p.badge || ''}, ${p.category || ''},
          ${p.colours || '[]'}, ${p.blurb || ''}, ${p.photo}, ${p.stock}, ${p.active}, ${p.barcode || ''}, ${p.vendor_barcode_photo})
    ...
    for (const row of (content || [])) {
      await sql`INSERT INTO content (section, json) VALUES (${row.section}, ${JSON.stringify(row.json)})
        ON CONFLICT (section) DO UPDATE SET json = EXCLUDED.json`;
    }
    ...
    res.json({
      ok: true,
      migrated: {
        users: (users || []).length, categories: (categories || []).length, products: (products || []).length,
        content: (content || []).length, reviews: (reviews || []).length, bookings: (bookings || []).length, orders: (orders || []).length
      }
    });
  } catch (err) {
    console.error(err);
